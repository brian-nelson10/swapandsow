...
const PostList = ({ posts, title, user }) => {
  ...
  return (
    <div className='py-8 flex items-center justify-center rounded-xl h-vh grid'>
      <h3 className='font-lofi text-[3rem] text-start justify-start'>{title}</h3>
      {posts &&
        posts.map(post => (
          <div key={post._id} className="card mb-3 p-8 shadow-xl bg-gray-200 rounded-lg grid items-center justify-center">
            ...
            <div className="card-header ml-2 -mt-1 font-lofi">
              <span className='block leading-snug'>
            <Link
                to={`/profile/${post.username}`}
                style={{ fontWeight: 700 }}
                className="text-black block font-lofi text-[2rem]"
            >
              {post.username}
              </Link>{' '}
              </span>
            ...
            <div className="card-body leading-snug font-lofi font-bold md:leading-normal">
            ...
            <div className='divide-y bg-gray-100'>
                ...
                <div className='p-4 grid grid-cols-2 font-lofi text-[1.3rem]'>
                  <div className='flex-row flex gap-4 mt-1'><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
