  ...
  return (
    <div className="upload-button">
      <div className="wrapper relative h-[4rem] w-[9rem] cursor-pointer overflow-hidden" onClick={() => !isAnimating && animate()}>
        <motion.div 
          className="container font-lofi upload absolute text-center justify-center flex items-center overflow-hidden text-[1.3rem] h-[100%] w-[100%] rounded-[8px] bg-[#ffd6a3] text-black font-bold py-2 px-4 rounded focus:outline-none focus:shadow-outline-blue hover:bg-[#00b4d8]"
          animate={uploadControls}
          whileHover="hovered">
          <motion.div variants={iconVariants}>{text}</motion.div>
        </motion.div>
        <motion.div  
          className={error ? "container done font-lofi absolute flex font-bold justify-center items-center overflow-hidden text-[1.3rem] h-[100%] w-[100%] rounded-[8px] text-black z-30 -top-[100%] bg-[red]" : "container done font-lofi absolute flex font-bold justify-center items-center overflow-hidden text-[1.3rem] h-[100%] w-[100%] rounded-[8px] text-black z-30 -top-[100%] bg-[#55a630]"}
          animate={doneControls}
        >
            <div>{error ? "error" : 'friends!'}</div>
        </motion.div>
      </div>
    </div>
  )
